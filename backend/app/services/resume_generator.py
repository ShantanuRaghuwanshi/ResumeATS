...
class EnhancedResumeGenerator:
    ...
    async def _generate_html(
        ...
        # Generate CSS based on tee
        css_styles = self._generate_css_styles(customizations, export_options.custtyling)
...
#f8f9fa;
        ...
        # Color scheme
        color_scheme = customizations.get
        color_map = {
f",
            "green": "#28a745",
42c1",
...
",
        ...
        .skill-category, .summary {{ border-left-col}
        """
...
d")
        if spacing == "compact":
            template_styles += """
            .section { margin: 15p
            .experience-item, .education-item, .project-item { marx 0; }
...
"wide":
        ...
        if cusg:
            if "primary_color" in custom_styling:
                custom_styles += f"""
                .name, .section-title, .item-title {{ color: {custom_s}}
                .header {{ border-bottom-color: {custom_stylin}}
                .skill-category, .summary {{ border-left-color: {custom_styling['prima}}
                """
            
tyling:
                custom_styles += f"body {{ font-size: 
            
            if "background_color" in custom_styling:
                custom_styles += f"body "
        
        return base_styles + template_styles + custom_styles

    def _generate_html_content(self, resume_r:
        """Generate HTML content for resu"
...
:
                contact_info.append(</a>')
            if personal.get("phone"):
             
            if personal.get("linkedin"):
                linkedin_url = personal["l"]
                if not linkedin_url.startswihttp"):
                    linkedin_url = f"https://{lin_url}"
                contact_info.append(f'<a href="{linkedina>')
            

            <div class="header">
                <div class="name">{personal.get('name')}</div>
                <div class="contact-info">{' | '.join(coniv>
            </div>
            """
...
[
        ...
        for section_name in section_order:
            if section_name in resume_data and resu:
                section_html = self._gener
                if section_html:
                    content_parts.append(section_html)
        
        return "\n".join(content_parts)

    def _generate_section_html(self, section_
...
())
        ...
        if section_name == "work_experience":
            ...
            return self._generate_educationon_data)
:
...
s"):
                achievements_list = "\n".join([f"<]])
                achievements_html = f'<ul class="achievementul>'
            
""
            if exp.get("from_date") or exp.get("to_date"):
                date_range = f'<div class="item-v>'
...
"
    ...
    def _generate_education_html(self, title: str, education: List[Dict[str, Any]]tr:
        """Ge
        ...
        for edu in education:
            ...
            item_html = f"""
>
                <div class="item-title">{edu.get('degree', '')}</iv>
                <div class="item-subtitle">{edu.get('un)}</div>
                {date_range}
            </div>
            """
            i)
...
>
    ...
    def _generate_skills_html(self, title: str, skills: List[Di
        """Generate HTML for skills section"""
        
 = []
        for skill_cat in skills:
            category_name = skill_cat.get("category",")
            skill_list = ", ".join(skill_cat.get(", []))
            

            <div class="skill-category">

                <div class="skill-list">{skill_lis>
            </div>
            """
...
""
    ...
    def _generate_pro -> str:
        ...
        for project in projects:
            bullets_html"
ts"):
                bullets_list = "\n".join([f"<li>{bullet}</li>" for bu)
</ul>'
            
            summary_html = ""
            if project.get("su
                summary_html = f'<div class="item-descript>'
        ...
        <div class="section">
            ...
            {"".join(items_html)}
>
        """  </div      /div>le}<">{tittitleon-ectitml}s_h    {bullet            di}</name', '')ject.get('le">{proy"]}</div"summar">{project[onimmary"):t}ullets_lis">{bievementsachlass="<ul c = f'htmlbullets_                "bullets"]]project[llet in ("bullegett.jecif pro            " = ]) Any]r,ct[st: List[Dijects, protitle: str, tml(selfects_hj "       >{title}e"n-titlectio"sclass=  <div           ""turn f        rediv</t}/div>}<ameory_n">{categy-titletegors="skill-ca  <div clas              l = f"""l_htm   skil         ls"skil "kills_html s       > str:Any]]) -ct[str,    </div     >item_htmlhtml.append(tems_y', ''iversitdation-item"ss="educ <div cla                   )}</d", ""earto_yu.get(")} - {ed, ""ear""from_yt(>{edu.gem-date"lass="ite<div cge = f'   date_ran        ion"""cation sectfor edunerate HTML ) -> sms_html)}ite{"".join(            title}</div">{ion-titlectses="iv clasd       ""     )}</company', ''>{exp.get('')}</di', 'title{exp.get('tle">em-tis="itclas       <div          e-iienc f"tml =item_h            mmary"]}</d["suion">{expem-descriptass="itiv cl)}</dient"Presate", "p.get("to_d"")} - {exom_date", p.get("fr>{ex"datee_range =     dat        ist}</s_lchievement">{asevements"p["achiin exchievement i>" for aievement}</lli>{achent"achievemet( exp.g         if   on""ectirience spe exML for worke HTnerat"""Ge        r:st, Any]]) -> ict[stres: List[Dnc, experietitle: str, ce_html(selfenexperiork_e_watgenerjects": "pro_data)ectiontml(title, ss_h= "skills"tion_name =    elif sec    title, sectitml(_ha) section_dattitle,l(htmrience_expework_ate__gener).title", " "eplace("_e.rection_nam sion_name,t(sect_titles.getionsec    title =         str:ta: Any) ->  section_da: str,nameon_name])a[sectiresume_datname, ection_n_html(ssectioate_me]on_nata[sectie_damprojectls", "on", "skilducati", ction_ordersens.get("ustomizatio_order = cction      se   from         /d}<tact_info)', '" f""html =ader_      he      ]}</kedin"["linrsonalk">{peet="_blan" targrl}_unkedith("nkedini])"phone(personal["ppendntact_info.a   coil"]}l["ema{persona">il"]}al["ema:{personmailtof="f'<a hre")mailal.get("eif person            []_info =    contact         ""meny]) -> stt[str, Azations: Dictomi], cust[str, Anya: Dicdatn}}\r']}; kground_colog['bacom_stylin{custcolor: d-un backgro{{']}; }}\n"_sizeling['fontom_sty{custin custom_sont_size"  "f  if          y_color']}; r; olor']}['primary_cg]}; color'g['primary_tylintom_stylin 0; }ing: 20pxpx 0; paddmargin: 25ect-item { em, .projcation-itdu.e, -itemexperiencepacing ==      elif s   px; }20padding: ainer { ont   .resume-c         g: 10pinpx 0; padd: 10ginx 0; }tandar"sspacing", et("ations.g = customizcing        spaor}; }cent_color: {ac_color};lor: {accent{ coem-title {title, .itn-sectio += fate_stylesmpl       "01f3f",y": "#0"nav             "#dc3545   "red":         : "#6f"purple"             "#007bf":     "blue        "blue")me",he("color_sc; }if, sans-sert_family}{fon: -familyy {{ font"bodes += femplate_styl       t  customizatiificspec        round: ackg      b      r: #2c3e5  colo         5pxer-radius:  bord          ill-categ    .sk    p: wraprax-w fle                  type: disctyle-em .achiev       r: #.item        66  color: #6           15p    padding:        0;c3e5 color: #2           ght: boweit-fon                     #66:.name {        om:ottom: margin-bott            0,0,0.1 0 a     margin:       6;ans-serif;ly: Arial, sfont-fami                    -> str:y]) r, Anict[styling: D custom_sty],ict[str, Anzations: Df, customies(sele_css_stylgeneratf _plemt
